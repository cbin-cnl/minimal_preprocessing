...
def run_pipelines(func_image, anat_image, output_directory):
    ...
    skullstrip, wm_segment, transform3, transform2, transform1, transform0 = ap.anat_pipeline(
        anat_image, str(tmp_anatomical)
    )
    motion_corrected, masked, masked_mean = fp.func_pipeline(
        func_image, str(tmp_functional)
    )
    warped = rp.main(
        skullstrip,
        motion_corrected,
