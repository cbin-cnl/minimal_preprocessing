...
def main(anatomical_path, func, mean, wm, trans3, trans2, trans1, trans0, output_path):
    ...
    if not Path(flirtmat).exists():
        registration_cmd = REGISTRATION.format(filepath=mean, anatpath=anatomical_path, outpath=flirtmat)
        run_cmd(env, registration_cmd)

    wmreg = str(mean).replace(".nii.gz", "_wmreg.mat")
    if not Path(wmreg).exists():
        wmreg_cmd = WM_REGISTRATION.format(filepath=mean, anatpath = anatomical_path, outpath=wmreg, wmpath=wm,
                                           initpath=flirtmat)
        run_cmd(env, wmreg_cmd)

    affine = str(output_path/"affine.txt")
    if not Path(affine).exists():
        affine_cmd = CONVERT_AFFINE.format(anatpath=anatomical_path, meanpath=mean, filepath=wmreg, outfile=affine)
    ...
    if not Path(warped).exists():
        warp_cmd = ANTS_APPLY_TRANSFORM.format(filepath=func, outpath=warped, transform3=trans3,
                                               transform2=trans2, transform1=trans1, transform0=trans0, affine=affine)
...
if __name__ == "__main__":
    ...
    func_path = (
        output_path / "20190109_114343MOVIEs009a001_resample_volregA.nii.gz"
    )
    func_mean_path = (
        output_path
    ...
    main(anat_path, func_path, func_mean_path, wm_path, transform3, transform2, transform1, transform0, output_path)
